  ...
  <div class="w-full relative">
    ...
    <div class="p-4 mb-4 bg-blue-50 border border-blue-200 rounded-xl shadow-sm">
      ...
      <p class="text-blue-700 mt-1 leading-relaxed">
        ...
        상단 검색창에서 <strong>병원명, 지역, 진료과</strong>로 검색하거나,<br>
        아래 버튼을 통해 <strong>지역 · 과목 필터링</strong>이 가능합니다.<br>
    ...
    <div class="p-4 bg-white shadow-md flex items-center space-x-3 rounded-lg mb-4">
      ...
      />
      <button @click="applyFilters" class="bg-sky-500 hover:bg-sky-600 text-white px-5 py-2 rounded-lg">
        검색
      </button>

      <button @click="moveToUserLocation" class="bg-gray-200 hover:bg-gray-300 px-4 py-2 rounded-lg">
        📍 현재위치
      </button>
    </div>
    ...
    <!--  진료과 버튼 -->
    <div class="flex flex-wrap gap-3 mb-4">
      <button class="px-4 py-2 bg-gray-300 rounded-lg" @click="resetDepartment">
        전체 과목
      </button>
    ...
    <div v-if="selectedHospital" class="hospital-panel">
      ...
      <p
        class="status-info"
        :class="{ holiday: selectedHospital.statusDetail !== '정상' }"
      >
        <span>
...
export default {
  ...
  methods: {
    loadScript() {
      ...
      script.src =
        `https://oapi.map.naver.com/openapi/v3/maps.js?ncpKeyId=${process.env.VUE_APP_NAVER_KEY_ID}`;
      script.onload = () => {
        this.initMap();
        this.loadHospitals();
      };
    ...
    async loadHospitals() {
      ...
      data = data.filter(
        (h) => h.x && h.y && (h.statusDetail === "정상" || h.statusDetail === "휴업")
      );
    ...
    showMyLocationMarker() {
      if (!this.userPos || !this.map) return;

      const pos = new naver.maps.LatLng(this.userPos.lat, this.userPos.lng);
    ...
    updateMarkers() {
      ...
      clusters.forEach((c) => {
        ...
        else this.createHospitalMarker(c.properties.hospital);
      });
    ...
    createHospitalMarker(hospital, highlight = false) {
      ...
      if (!highlight) this.markers.push(marker);
    ...
    selectArea(area) {
      ...
      this.map.setCenter(
        new naver.maps.LatLng(this.areaCenters[area].lat, this.areaCenters[area].lng)
      );
      this.map.setZoom(12);
      this.applyFilters();
    },
    ...
    moveToUserLocation() {
      ...
      this.map.setCenter(
        new naver.maps.LatLng(this.userPos.lat, this.userPos.lng)
      );
      this.map.setZoom(13);
      this.showMyLocationMarker();
    },
